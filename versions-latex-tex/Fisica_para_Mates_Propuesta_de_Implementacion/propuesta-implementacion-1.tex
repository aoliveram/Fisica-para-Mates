...
\author{
  Aníbal Olivera M. \\
  \texttt{an.oliveram@udd.cl}
  \and
  Viviana Solano P.\\
  \texttt{vsolano@udd.cl}
}
\date{Enero 2026}
...
\midrule
Sólido Rev. 2 (5.1)  &   & X & X &   & X & X & X & X &   &   & X & 2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
   \item[0:] Álgebra
  \item[1:] Geometría
  \item[2:] Taller de Física I o Introducción al Cálculo
\end{itemize}
...
\item Para esta propuesta se utilizarán sesiones de 140 minutos, agendadas al final del \textbf{primer módulo} del curso (de un total de 2). 
\end{itemize}
...
Para medir el impacto de la intervención en variables de motivación y estrategias de aprendizaje se utilizará la versión del cuestionario \textbf{MSLQ} presentado por Inzunza et al. (2018), validado en estudiantes universitarios chilenos de primer año. Este instrumento no mide contenidos específicos de una asignatura, sino constructos generales de motivación para el aprendizaje (valoración de la tarea, expectativas/autoeficacia y componente afectivo como ansiedad ante evaluaciones). Eso lo hace portable: basta con contextualizar la redacción (``este curso'', ``esta asignatura'') a Física para Mates y mantener el formato de respuesta.

En su versión completa, el instrumento contiene alrededor de 80 ítems tipo Likert. Sin embargo, el módulo de \textit{motivación} de Inzunza et al. (2018) consta solo de 28 ítems. En adelante, se detalla el plan de aplicación y el banco de ítems seleccionado.
...
\section{Herramienta para medir Motivación}

El objetivo es construir una medida breve de ``motivación académica'' usando solo los ítems que, en la adaptación chilena reportada por Inzunza et al. (2018), conforman los 3 factores motivacionales (valoración, expectativas y afecto/ansiedad), y luego validar internamente esa estructura en la muestra. Siguiendo esta estrategia, seleccionamos los factores e ítems que se detallan a continuación.
...
El total de ítems a aplicar es de 28 ítems (F1=9, F3=14, F8=5), a ser medidos usando una escala Likert de 1 a 5 (total desacuerdo a total acuerdo), manteniendo el período de referencia ``este curso''. En cuanto a tiempos de aplicación, se estiman los siguientes:
\begin{itemize}
...
Para tener evidencia de validez y confiabilidad en la muestra del curso, se hará el siguiente análisis. En la muestra, ejecutar:
\begin{enumerate}
    ...
    \item CFA (Análisis Factorial Confirmatorio):
    \begin{itemize}
